...
export default function TradeDetail({ tradeId }: TradeDetailPageProps) {
  const { user } = useAuth();
  ...
  const { mutate: updateTradeVisibility } = useMutation({
    mutationFn: async (visibility: 'public' | 'private') => {
      const response = await apiRequest('PUT', `/api/trades/${tradeId}`, { visibility });
      return response.json();
    },
    onSuccess: () => {
      // Invalidate trade queries to refresh data
      queryClient.invalidateQueries({ queryKey: ['/api/trades', tradeId] });
      toast({
        title: "Success",
        description: "Trade visibility updated successfully",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  // Set initial screenshot display and visibility based on available data
  useEffect(() => {
    if (trade) {
      // Always show screenshot area, even if no image available
      setShowScreenshot(true);
      // Set visibility from trade data, default to 'private' if not specified
      setTradeVisibility((trade as any).visibility || 'private');
    }
  }, [trade]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      <Navigation />
      <MobileNav onNewTrade={() => setShowNewTradeModal(true)} onLogEmotion={() => {}} />
                ...
                <div className="hidden lg:flex items-center space-x-3">
                  {tradeVisibility === 'public' && (
                    <Button
                      ...
                      onClick={() => {
                        // Generate shareable link with actual trade ID
                        const shareableLink = `${window.location.origin}/shared/${user?.username}/${trade.id}`;
                      ...
                      data-testid="button-share-trade"
                    >
                      <Share2 className="h-4 w-4" />
                      <span>Share</span>
                    </Button>
                  )}
                  <Select value={tradeVisibility} onValueChange={(value: 'public' | 'private') => {
                    setTradeVisibility(value);
                    updateTradeVisibility(value);
                  }}>
              ...
              <CardContent className="p-4">
                ...
                {tradeVisibility === 'public' && (
                  <Button
                    ...
                    data-testid="button-share-trade-mobile"
                  >
                    <Share2 className="h-4 w-4" />
                ...
                <Card className="bg-card border border-border shadow-xl h-fit mb-4">
                  <CardHeader>
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      <span>{symbol} Trade Screenshot</span>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="h-64 md:h-96 flex items-center justify-center bg-muted rounded-lg relative">
                      {trade?.imageUrl ? (
                        <>
                          <img 
                            src={trade.imageUrl} 
